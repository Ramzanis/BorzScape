...
public class EconomyManager {
	...
	private static String[] SHOPS_NAMES = new String[] { "General store", "Vote items 1", "Vote items 2", "Vote items 3", "Melee", "Ranged", "Magic", "Food & Potions", "Rings & Amulets & Gloves", "Capes", "Skilling items 1", "Skilling items 2", "Herblore Secondaries 1", "Herblore Secondaries 2", "Back" };
	private static int[] SHOPS_IDS = new int[] { 200, 209, 212, 213, 201, 202, 203, 204, 205, 206, 207, 208, 210, 211, -1 };
	...
	private static String[] DUNGEON_NAMES = new String[] { "Stronghold of security", "Karamja & Crandor", "Brimhaven dungeon", "TzHaar", "Jungle Strykewyrms", "Desert Skrykewyrms", "Ice Strykewyrms", "Kalphite hive", "Asgarnia ice dungeon", "Mos le harmless jungle", "Gorak", "Lumbridge swamp caves", "Grotworm lair (QBD)", "Framenik slayer dungeon", "King black dragon", "Corporeal beast", "Tormented demons", "God Wars", "Back" };
	private static WorldTile[] DUNGEON_LOCATIONS = new WorldTile[] { new WorldTile(3080, 3418, 0), new WorldTile(2861, 9570, 0), new WorldTile(2745, 3152, 0), new WorldTile(4673, 5116, 0), new WorldTile(2450, 2898, 0), new WorldTile(3381, 3162, 0), new WorldTile(3508, 5516, 0), new WorldTile(3228, 3106, 0), new WorldTile(3010, 3150, 0), new WorldTile(3731, 3039, 0), new WorldTile(3035, 5346, 0), new WorldTile(3169, 3171, 0), new WorldTile(2990, 3237, 0), new WorldTile(2794, 3615, 0), new WorldTile(3051, 3519, 0), new WorldTile(2966, 4383, 2), new WorldTile(2562, 5739, 0), new WorldTile(2857, 3573, 0), null };
	...
	private static String[] OTHER_NAMES = new String[] { "Mages bank", "Multi pvp (Wilderness)", "Wests (Wilderness)", "Easts (Wilderness)", "Back" };
	private static WorldTile[] OTHER_LOCATIONS = new WorldTile[] { new WorldTile(2538, 4715, 0), new WorldTile(3240, 3611, 0), new WorldTile(2984, 3596, 0), new WorldTile(3360, 3658, 0), null };
			...
			CoresManager.fastExecutor.schedule(new TimerTask() {
				...
				public void run() {
					...
					for (NPC npc : World.getNPCs()) {
						...
						String message = "An " + eventText + " is currently happening! Talk to Oracle of Dawn to get there!";
	...
	public static void processManagerNpcClick(final Player player, final int npcId) {
		player.getDialogueManager().startDialogue(new Dialogue() {
			...
			private void handlePage(int optionId) {
				if (pageId == 0) { // title page
					if (optionId == 0) // information & links
						...
						setPage(5, "This section contains links to our vote page, as well as ability to claim your reward.", "Vote", "Claim your reward", "Back");
					...
					if (optionId == 0) { // change your password
						player.getTemporaryAttributtes().put("change_pass", true);
						player.getPackets().sendInputLongTextScript("Enter your new password:");
					} else if (optionId == 1) { // auth forum acc
						player.getTemporaryAttributtes().put("forum_authuserinput", true);
						player.getPackets().sendInputNameScript("Enter your forum username:");
			...
			private void setTitlePage() {
				setPage(0, "Welcome to " + Settings.SERVER_NAME + "!<br>I provide various services to make your life here easier.", "Information & Links", "Account & Character management", eventHappening ? "Teleports (Click here for event)" : "Teleports", "PK Ranks", "Shops", "Vote", "Donate", "Submit a ticket", "Nevermind");
			}
			...
			private void setTeleportsTitlePage() {
				setPage(3, "This section contains teleports to various different locations.", "Current event", "Current starting town", "Safe PvP", "Recommended for beginners", "Cities & Towns", "Dungeons & PVM Locations", "Minigames", "Others", "Back");
			}
