...
public class Prayer implements Serializable {
	...
	public void increaseTurmoilBonus(Entity entity) {
		if (entity instanceof Player) {
			 ...
			 * NPC npc = (NPC) entity;
			 * 
			 * leechBonuses[8] = (int) ((100 * Math.floor(0.15 * npc.getBonuses()[CombatDefinitions.STAB_ATTACK])) /
			 * npc.getBonuses()[CombatDefinitions.STAB_ATTACK]); leechBonuses[9] = (int) ((100 * Math.floor(0.15 *
			 * npc.getBonuses()[CombatDefinitions.STAB_DEF])) / npc.getBonuses()[CombatDefinitions.STAB_DEF]); leechBonuses[10] = (int)
			 * ((100 * Math.floor(0.1 * npc.getBonuses()[CombatDefinitions.CRUSH_ATTACK]))) /
			 * npc.getBonuses()[CombatDefinitions.CRUSH_ATTACK]; adjustStat(0, leechBonuses[8]); adjustStat(1, leechBonuses[10]);
			 * adjustStat(2, leechBonuses[9]);
			 */
		}
	}
		...
		for (int index = 0; index < onPrayers[prayerBook].length; index++) {
			if (onPrayers[prayerBook][index]) {
				...
				if (drainTimer != 0 && drainTimer <= currentTime) {
					int rate = (int) ((prayerDrainRate[getPrayerBook()][index] * 10000) + (prayerPoints * 65));
	...
	public void resetDrainPrayer(int index) {
		nextDrain[index] = (long) (Utils.currentTimeMillis() + (prayerDrainRate[getPrayerBook()][index] * 10000) + (player.getCombatDefinitions().getBonuses()[CombatDefinitions.PRAYER_BONUS] * 65));
	}
	...
	public void unlockPrayerBookButtons() {
		player.getPackets().sendIComponentSettings(271, usingQuickPrayer ? 42 : 8, 0, 26, 2);
	}
	...
	public void addPrayerpoints(int pp, boolean boost) {
		int maxPrayer = player.getSkills().getLevelForXp(Skills.PRAYER);
		if (!boost) {
			if ((prayerpoints + pp) < maxPrayer)
				prayerpoints += pp;
			else
				prayerpoints = maxPrayer;
		} else {
			if ((prayerpoints + pp) >= (maxPrayer + pp)) {
				prayerpoints = (maxPrayer + pp);
				refreshPrayerPoints();
				return;
			}
			prayerpoints += pp;
	...
	public void refreshPrayerPoints() {
		player.getSkills().set(Skills.PRAYER, prayerpoints);
