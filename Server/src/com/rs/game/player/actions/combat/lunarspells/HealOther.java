	...
	public static boolean cast(Player player, Entity target, double xp) {
		if (target instanceof Player) {
			...
			if ((Long) player.getTemporaryAttributtes().get("LAST_SPELL") != null
				...
				player.getSocialManager().sendGameMessage("You can't do this yet.");
				return false;
			}
			...
			if (other.getHitpoints() != other.getMaxHitpoints()) {
				...
				other.getSocialManager().sendGameMessage("You have been healed by player " + player.getDisplayName() + ".");
				...
				player.getSocialManager().sendGameMessage(other.getDisplayName() + " has already full hitpoints.");
				return false;
			}
		}
		return false;
