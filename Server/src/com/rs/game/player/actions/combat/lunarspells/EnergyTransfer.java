...
public class EnergyTransfer {
	...
	public static boolean cast(Player player, Entity target, double xp) {
		if (target instanceof Player) {
			...
			if (!other.isAcceptingAid()) {
				player.getSocialManager().sendGameMessage(other.getDisplayName() + " doesn't have aid on.");
				return false;
			}
			if (other.getCombatDefinitions().getSpecialAttackPercentage() == 100) {
				player.getSocialManager().sendGameMessage(other.getDisplayName() + " has full special attack.");
				return false;
			}
			if (player.getCombatDefinitions().getSpecialAttackPercentage() == 0) {
				player.getSocialManager().sendGameMessage("You don't have any special attack left.");
				return false;
			}
			...
			if (amount > player.getCombatDefinitions().getSpecialAttackPercentage())
				...
				other.getSocialManager().sendGameMessage("You got an energy transfer from player " + player.getDisplayName() + ".");
