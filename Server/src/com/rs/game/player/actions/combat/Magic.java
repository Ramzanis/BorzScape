...
public class Magic {
	...
	public static final void processNormalSpell(Player player, int spellId, int packetId) {
		...
		case 67:
		case 68:
			//TODO if (player.getEquipment().getWeaponId() == 24203 && player.getRunicStaff().getSpellId() == spellId
		...
		case 33:
			...
			if (amountOfBones == 0) {
				player.getSocialManager().sendGameMessage("You don't have any bones.");
				return;
			}
			if (!Magic.checkRunes1(player, true, Magic.EARTH_RUNE, 2, Magic.WATER_RUNE, 2, Magic.NATURE_RUNE, 1)) {
				player.getSocialManager().sendGameMessage("You do not have enough runes for this spell.");
				return;
			}
		...
		case 83:
			if (player.getSkills().getLevel(Skills.MAGIC) < 80) {
				player.getSocialManager().sendGameMessage("You need at least a level of 80 magic to cast this spell.");
				return;
			}
			if (!player.getInventory().containsItem(565, 3) || !player.getInventory().containsItem(554, 3)
					|| !player.getInventory().containsItem(556, 3)) {
				player.getSocialManager().sendGameMessage("You do not have the required runes to cast this spell.");
				return;
			}
			if ((player.getEquipment().getWeaponId() != 2415 && player.getEquipment().getWeaponId() != 2416
					&& player.getEquipment().getWeaponId() != 2417)) {
				player.getSocialManager().sendGameMessage("You do not have the required equipment to cast this spell.");
				return;
			} else {
			...
			if (player.getChargeImmune() > Utils.currentTimeMillis()) {
				player.getSocialManager().sendGameMessage("You can't use this spell yet.");
				return;
			} else {
	...
	private static void useHomeTele(Player player) {
		if (player.isAtWild() || player.isCanPvp()) {
			...
			player.stopAll();
			if (player.getCombatDefinitions().getSpellBook() == 2) {// lunar
				sendLunarTeleportSpell(player, 0, 0, new WorldTile(Settings.HOME_PLAYER_LOCATION));
			} else if (player.getCombatDefinitions().getSpellBook() == 0) {// normal
				sendNormalTeleportSpell(player, 0, 0, new WorldTile(Settings.HOME_PLAYER_LOCATION));
			} else if (player.getCombatDefinitions().getSpellBook() == 1) {// ancient
	...
	public static final boolean checkMiscRunes(Player player, boolean delete, int spellId, int... runes) {
		...
		while (runesCount < runes.length) {
			...
			//TODO if (player.getRunePouch().contains(rune) && player.getInventory().containsOneItem(24497))
			//	continue;
		...
		if (hasStaffOfLight(weaponId) && Utils.getRandom(2) == 0) {
			delete = false;
			player.getSocialManager().sendGameMessage("Your spell draws its power completely from your staff.");
		}
		if (delete) {
			runesCount = 0;
			while (runesCount < runes.length) {
	...
	public static final boolean checkRunes1(Player player, boolean delete, int... runes) {
		...
		while (runesCount < runes.length) {
			...
			if (!player.getInventory().containsItem(runeId, ammount)) {
				...
				player.getSocialManager().sendGameMessage("You do not have enough "
								+ ItemDefinitions.getItemDefinitions(runeId).getName().replace("rune", "Rune")
								+ "s to cast this spell.");
				return false;
			}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (stage == 0) {
					...
					WorldTile teleTile = /*//TODO player.getControlerManager().getControler() instanceof EdgevillePvPControler
					...
					player.setNextWorldTile(
							/*player.getControlerManager().getControler() instanceof EdgevillePvPControler ? pvp //TODO
									: */teleTile);
					player.getControlerManager().magicTeleported(ITEM_TELEPORT);
					if (player.getControlerManager().getControler() == null)
						teleControlersCheck(player, teleTile);
					...
					player.setFreezeDelay(-1);
					//TODO if (!(player.getControlerManager().getControler() instanceof EdgevillePvPControler))
					//	player.getControlerManager().forceStop();
					stop();
				}
			}
	...
	public static void useEctoPhial(final Player player, Item item) {
		player.lock(6);
		player.getSocialManager().sendGameMessage("You empty the ectoplasm onto the floor...");
	...
	public static void spyPlayer(final Player player, final Player other) {
		...
		if ((Long) player.getTemporaryAttributtes().get("LAST_SPELL") != null
				&& (Long) player.getTemporaryAttributtes().get("LAST_SPELL") + 8250 > Utils.currentTimeMillis()) {
			return;
		}
	...
	public static final boolean checkRunes(Player player, boolean delete, boolean dungeoneering, int... runes) {
		...
		while (runesCount < runes.length) {
			...
			if (hasInfiniteRunes(runeId, weaponId, shieldId))
				...
				player.getSocialManager().sendGameMessage("The power of your staff of light saves some runes from being drained.");
				continue;
