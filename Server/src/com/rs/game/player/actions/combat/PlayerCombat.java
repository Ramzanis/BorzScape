...
public class PlayerCombat extends Action {
    ...
    public int processWithDelay(Player player) {
        ...
        if (player.getTemporaryAttributtes().get("miasmic_effect") == Boolean.TRUE)
            multiplier = 1.5;
        if (player.getTemporaryAttributtes().get("bakriminel_effect") == Boolean.TRUE)
            magicMultiplier = 1.5;
...
//
    ...
    public int mageAttack(final Player player, int spellId, boolean autocast) {
        ...
        /*f (spellId == player.getRunicStaff().getSpellId() && hasRunicStaffCharges(player)
        ...
        if (player.getCombatDefinitions().getSpellBook() == ModernMagicks.SPELLBOOK_ID) {
            ...
            if (spell == ModernCombatSpellsStore.CRUMBLE_UNDEAD) {
                ...
                if (target instanceof NPC) {
                    ...
                    for (String string : NpcDamageBoost.UNDEAD_NPCS) {
            ...
            if (spell == ModernCombatSpellsStore.TELEPORT_BLOCK) {
                if (target instanceof NPC) {
                    player.getSocialManager().sendGameMessage("You can't cast teleport block on a monster.");
                    return -1;
                }
                if (target instanceof Player) {
                    Player p2 = (Player) target;
                    if (p2.getTeleBlockDelay() > Utils.currentTimeMillis()) {
                        player.getSocialManager().sendGameMessage("This player is already teleport blocked.");
                        return -1;
                    }
                }
            }
            ...
            if (spell.isMulti_spell() && player.isAtMultiArea()) {
                ...
                /*if (target instanceof Player) {
                    ...
                    if (p2.getEquipment().getWeaponId() == 24203 && p2.getRunicStaff().getSpellId() == spell.getId()) {
                        if (Utils.getRandom(4) == 0) {
                            hit.setDamage(-2);
                            player.getSocialManager().sendGameMessage("Enemies greater runic staff resists your "
                                    + spell.name().toLowerCase().replace("_", " ") + " spell.");
                        } else if (Utils.getRandom(15) == 0) {
                            p2.getRunicStaff().chargeCombat(1);
                            player.getSocialManager().sendGameMessage(p2.getDisplayName() + " absorbs your "
                                    + spell.name().toLowerCase().replace("_", " ") + " spell.");
                            p2.getPackets()
    ...
    public int getMagicMaxHit(Player player, int baseDamage) {
        ...
        if (spellcasterGloves > 0) {
            if (baseDamage > 60 || spellcasterGloves == 28 || spellcasterGloves == 25) {
                ...
                if (target instanceof Player) {
                    Player p2 = (Player) target;
                    p2.getSkills().drainLevel(Skills.ATTACK, (int) (p2.getSkills().getLevel(Skills.ATTACK) * 0.9));
                    p2.getSkills().drainLevel(Skills.DEFENCE, (int) (p2.getSkills().getLevel(Skills.DEFENCE) * 0.9));
                    p2.getSkills().drainLevel(Skills.STRENGTH, (int) (p2.getSkills().getLevel(Skills.STRENGTH) * 0.9));
    ...
    private int rangeAttack(final Player player) {
        ...
        if (player.getCombatDefinitions().isUsingSpecialAttack()) {
            ...
            if (specAmt == 0) {
                player.getSocialManager().sendGameMessage("This weapon has no special Attack.");
            ...
            if (weaponId != -1) {
                ...
                if (weaponName.toLowerCase().contains("throwing axe") || weaponName.toLowerCase().contains("knife")
                    ...
                    player.setNextAnimationNoPriority(new Animation(2779));
                        ...
                        switch (ammoId) {
                            ...
                            case 24116:
                                ...
                                if (attackStyle == 0) {
                                    ...
                                    if (target.getTemporaryAttributtes().get("bakriminel_immunity") != Boolean.TRUE) {
                                        if (target instanceof Player) {
                                            ((Player) target).getSocialManager().sendGameMessage("You feel slowed down.");
                                        }
                                        target.getTemporaryAttributtes().put("bakriminel_immunity", Boolean.TRUE);
                                        target.getTemporaryAttributtes().put("bakriminel_effect", Boolean.TRUE);
                                        final Entity t = target;
                                        WorldTasksManager.schedule(new WorldTask() {
                                            @Override
                                            public void run() {
                                                t.getTemporaryAttributtes().remove("bakriminel_effect");
                                                WorldTasksManager.schedule(new WorldTask() {
                                                    @Override
                                                    public void run() {
                                                        t.getTemporaryAttributtes().remove("bakriminel_immunity");
                                                        stop();
                                                    }
                                                }, 15);
                                                stop();
                ...
                player.setNextAnimationNoPriority(new Animation(getWeaponAttackEmote(weaponId, attackStyle)));
            }
        }
        playSound(soundId, player, target);
        return combatDelay;
        ...
        if (quantity == -2) {
            ...
            if (weaponId != -1) {
                if ((capeId == 10498 || capeId == 10499 || capeId == 20068 || capeId == 20769 || capeId == 20771)
                        && Utils.getRandom(2) <= 1) {
                    /*
                     * player.getSocialManager().sendGameMessage( "Your " +
    ...
    private int meleeAttack(final Player player) {
        ...
        if (player.getCombatDefinitions().isUsingSpecialAttack()) {
            ...
            switch (weaponId) {
                ...
                case 24510:
                case 23679:
                    player.setNextAnimationNoPriority(new Animation(11989));
                ...
                case 14484:
                case 23695:
                    player.setNextAnimationNoPriority(new Animation(10961));
                ...
                case 5698: // dds
                    player.setNextAnimationNoPriority(new Animation(1062));
    ...
    public void sendSap(final Player player, final Entity target) {
        if (target instanceof Player) {
            ...
            if (Utils.getRandom(4) == 0 && projectileId != -1) {
                player.setNextAnimationNoPriority(new Animation(12569));
                ...
                if (usingMelee) {
                    ...
                    if (reachedMax)
                        ...
                        p2.getSocialManager().sendGameMessage("Your Melee has been drained by an enemy curse.", true);
                    }
                }
                if (usingRange) {
                    if (player.getPrayer().reachedMax(3) || player.getPrayer().reachedMin(3))
                        ...
                        p2.getSocialManager().sendGameMessage("Your Ranging has been drained by an enemy curse.", true);
                    }
                }
                if (usingMagic) {
                    if (player.getPrayer().reachedMax(4) || player.getPrayer().reachedMin(4))
                        ...
                        p2.getSocialManager().sendGameMessage("Your Magic has been drained by an enemy curse.", true);
                    }
                }
                if (usingSpecial) {
                    if (p2.getCombatDefinitions().getSpecialAttackPercentage() <= 0)
                        ...
                        player.getSocialManager().sendGameMessage("Your curse drains Special Attack from the enemy.", true);
                        p2.getSocialManager().sendGameMessage("Your Special Attack has been drained by an enemy curse.",
                                true);
                    }
                }
            }
    ...
    public void sendLeech(final Player player, final Entity target) {
        if (target instanceof Player) {
            ...
            if (Utils.getRandom(4) == 0 && getCurseId != -1) {
                player.setNextAnimationNoPriority(new Animation(12575));
                ...
                p2.getSocialManager().sendGameMessage("Your " + message + " has been drained by an enemy curse.", true);
                if (usingAttack) {
        ...
        if (target instanceof Player) {
            if (!player.getPrayer().isBoostedLeech()) {
                if (player.getPrayer().usingPrayer(1, 19)) {
                    player.getPrayer().increaseTurmoilBonus(p2);
                    // player.getPrayer().setBoostedLeech(true);
                }
            }
        }
    ...
    public void boostTurmoilNPC(final Player player) {
        if (target instanceof NPC) {
            if (!player.getPrayer().isBoostedLeech()) {
                if (player.getPrayer().usingPrayer(1, 19)) {
                    player.getPrayer().increaseTurmoilBonus(player);
    ...
    public void handleDisruptionShield(Player player, Hit hit) {
        if (target instanceof Player) {
            ...
            if (p2.isDisruptionActivated() && hit.getDamage() >= 1 && (hit.getLook() == HitLook.MELEE_DAMAGE
                ...
                hit.setMissedHit();
                p2.getSocialManager().sendGameMessage("Your Disruption Shield blocked your victim's damage.");
                player.getSocialManager().sendGameMessage("Your victim's Disruption Shield blocked your damage.");
        ...
        if (target instanceof Player) {
            ...
            if (player.getEquipment().getGlovesId() == 20694) {
                if (Utils.random(2) == 1 && hit.getDamage() > 100) {
                    ...
                    p2.getSocialManager().sendGameMessage("Your opponents' deathtouch bracelet activiates and hits <col=ff0000>" + reflection
                                    + "</col> damage.");
    ...
    public void delayHit(int delay, final int weaponId, final int attackStyle, final Hit... hits) {
        ...
        for (Hit hit : hits) {
            ...
            if (target instanceof Player) {
                ...
                //if (!p2.getAttackedBy().containsKey(player))
                  //TODO  p2.getAttackedBy().put(player, 1440);// 15minutes add to list
                if (player.getEquipment().getWeaponId() == 4566)
                ...
                sendLeech(player, p2);
                sendSap(player, p2);
                //TODO p2.getCharges().processIncommingHit();
            }
            //TODO player.getCharges().processOutgoingHit();
            ...
            if (target instanceof NPC) {
                ...
                if (n.getId() == 1158) {
                    if (rangeAttack || magicAttack) {
                        hit.setDamage(0);
                        player.getSocialManager().sendGameMessage(n.getName() + "s protection prayer blocks your "
                ...
                if (n.getName().toLowerCase().contains("turoth") || n.getName().toLowerCase().contains("kurask")) {
                    if (meleeAttack) {
                        ...
                        if (player.getEquipment().getAmmoId() != 4160 && player.getEquipment().getAmmoId() != 13280) {
                            player.getSocialManager().sendGameMessage("You can't harm " + n.getName() + "s with this ammo.");
                            hit.setDamage(0);
                        }
                    } else if (magicAttack) {
                        ...
                        if (splash || hit.getDamage() == 0) {
                            ...
                            if (block_tele) {
                                if (target instanceof Player) {
                                    ...
                                    targetPlayer.getSocialManager().sendGameMessage(
        ...
        if (!(target instanceof NPC && ((NPC) target).isForceMultiAttacked())) {
            if (player.isAtMultiArea() && !target.isAtMultiArea()) {
                if (target.getAttackedBy() != player && target.getAttackedByDelay() > Utils.currentTimeMillis()) {
                    player.getSocialManager().sendGameMessage("That "
            ...
            if (player.getCombatDefinitions().isUsingSpecialAttack()) {
                ...
                switch (weaponId) {
                    case 4153:
                        ...
                        player.setNextAnimationNoPriority(new Animation(1667));
                        player.gfx(new Graphics(340, 0, 96 << 16));
                        delayNormalHit(weaponId, attackStyle, getMeleeHit(player,
                        ...
                        break;
                    case 14679:
                        player.setNextAnimationNoPriority(new Animation(10505));
                        player.gfx(new Graphics(1840));
                        player.faceEntity(target);
                        delayNormalHit(weaponId, attackStyle, getMeleeHit(player,
                                getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.0, true)));
