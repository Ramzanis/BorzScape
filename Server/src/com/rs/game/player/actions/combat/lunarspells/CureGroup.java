...
public class CureGroup {
	...
	public static boolean cast(Player player, double xp) {
		Long lastCureGroup = (Long) player.getTemporaryAttributtes().get("LAST_CUREGROUP");
		...
		for (Player other : World.getPlayers()) {
			if (other.withinDistance(player, 4) && other.isAcceptingAid() && other.isAtMultiArea()) {
				if (!other.getUsername().equalsIgnoreCase(player.getUsername())) {
					...
					other.getTemporaryAttributtes().put("LAST_CUREGROUP", Utils.currentTimeMillis());
					playersAround++;
		...
		player.getTemporaryAttributtes().put("LAST_CUREGROUP", Utils.currentTimeMillis());
		return true;
	}
}
