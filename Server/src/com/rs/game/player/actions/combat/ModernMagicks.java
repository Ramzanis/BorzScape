...
import com.rs.game.player.actions.combat.modernspells.*;
...
public class ModernMagicks {
	...
	//TODO public static boolean hasChargedRunicStaff(Player player, int spellId) {
	...
	public static void handleFloorSpell(Player player, int spellId, FloorItem floorItem) {
		...
		switch (s) {
			case TELEGRAB:// TODO TELEGRAB, GET PACKET
				player.getSocialManager().sendGameMessage("telegrab handler works");
				break;
		}
	}
	...
	public static void handleTargetSpell(Player player, int spellId, Entity target) {
		...
		switch (s) {
			...
			case TELEPORT_OTHER_FALADOR:
				player.getSocialManager().sendGameMessage("Teleport " + p2.getDisplayName() + " to " + s.name());
				break;
		}
	}
