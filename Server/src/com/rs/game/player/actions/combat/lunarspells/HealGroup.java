...
public class HealGroup {
	...
	public static boolean cast(Player player, double xp) {
		Long lastHealGroup = (Long) player.getTemporaryAttributtes().get("LAST_HEALGROUP");
		...
		for (Player other : World.getPlayers()) {
			if (other == null)
				continue;
			if (other.withinDistance(player, 4) && other.isAcceptingAid()) {
				if (!other.getUsername().equalsIgnoreCase(player.getUsername())) {
					...
					other.getTemporaryAttributtes().put("LAST_HEALGROUP", Utils.currentTimeMillis());
					playersAround++;
		...
		player.getTemporaryAttributtes().put("LAST_HEALGROUP", Utils.currentTimeMillis());
		return true;
	}
}
