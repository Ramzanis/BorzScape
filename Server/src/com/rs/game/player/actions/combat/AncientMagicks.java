...
public class AncientMagicks {
	...
	public static boolean hasChargedRunicStaff(Player player, int spellId) {//TODO
	...
	private static boolean hasRune(Player player, int itemId, int amount) {//TODO
		return player.getInventory().containsItem(itemId, amount);/* || player.getRunePouch().contains(new Item(itemId, amount))*/
	}
					...
					for (Item staff : s.getStaff()) {
						...
						if (staffAmount == 1) {
							if (player.getEquipment().getWeaponId() != staff.getId()) {
								player.getSocialManager().sendGameMessage("You need to have "
