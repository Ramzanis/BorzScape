		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (weapon.getDefinitions().getName().contains("crossbow") || weapon.getDefinitions().getName().contains("c'bow")) {
					...
					p.setNextGraphics(new Graphics(PlayerCombat.getThrowProjectileId(weapon.getId()), 0, 100));
					World.sendProjectile(p, o, PlayerCombat.getThrowProjectileId(weapon.getId()), 41, 36, 41, 32, 15, 0);
