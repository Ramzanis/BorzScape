...
public final class Skills implements Serializable {
	...
	public static final int ATTACK = 0, DEFENCE = 1, STRENGTH = 2, HITPOINTS = 3, RANGE = 4, PRAYER = 5, MAGIC = 6, COOKING = 7, WOODCUTTING = 8, FLETCHING = 9, FISHING = 10, FIREMAKING = 11, CRAFTING = 12, SMITHING = 13, MINING = 14, HERBLORE = 15, AGILITY = 16, THIEVING = 17, SLAYER = 18, FARMING = 19, RUNECRAFTING = 20, CONSTRUCTION = 22, HUNTER = 21, SUMMONING = 23;

	public static final String[] SKILL_NAME = { "Attack", "Defence", "Strength", "Constitution", "Ranging", "Prayer", "Magic", "Cooking", "Woodcutting", "Fletching", "Fishing", "Firemaking", "Crafting", "Smithing", "Mining", "Herblore", "Agility", "Thieving", "Slayer", "Farming", "Runecrafting", "Hunter", "Construction", "Summoning" };
	...
	public boolean hasRequiriments(int... skills) {
		for (int i = 0; i < skills.length; i += 2) {
			...
			if (skillId == CONSTRUCTION || skillId == SLAYER)
				continue;
	...
	/*
	 * if(componentId == 33) setCounterSkill(4); else if(componentId == 34) setCounterSkill(2); else if(componentId == 35)
	 * setCounterSkill(3); else if(componentId == 42) setCounterSkill(18); else if(componentId == 49) setCounterSkill(11);
	 */

	public double addXp(int skill, double exp) {
		int rate = skill == ATTACK || skill == STRENGTH || skill == DEFENCE || skill == HITPOINTS || skill == MAGIC || skill == RANGE || skill == SUMMONING ? Settings.COMBAT_XP_RATE : Settings.XP_RATE;
	...
	public double addXpNoBonus(int skill, double exp) {
		player.getControlerManager().trackXP(skill, (int) exp);
		if (player.isXpLocked())
			return 0;
		int oldLevel = getLevelForXp(skill);
	...
	public void refreshXpCounter() {
		if (xpCounter > 99999999) {
			xpCounter = 99999999;
		}
		player.getVarsManager().sendVar(1801, xpCounter * 10);
	}
