...
public class NPC extends Entity implements Serializable {
	...
	public void sendLootshare(Player player, Item item) {
		...
		if (fc != null) {
			...
			for (Player fcPlayers : fc.getPlayers()) {
				if (fcPlayers == null)
					continue;
				players.add(World.getPlayerByDisplayName(String.valueOf(fcPlayers)));
			}
			CopyOnWriteArrayList<Player> playersWithLs = new CopyOnWriteArrayList<Player>();
			...
			if (item.getAmount() > 0) {
				...
				if ((item.getDefinitions().getTipitPrice() * item.getAmount()) >= luckyPlayer.getValueableDrop()) {
					luckyPlayer.getSocialManager().sendGameMessage("<col=ff0000>Valuable drop: " + item.getName()
				...
				if (!item.getDefinitions().isTradeable() && !item.getName().toLowerCase().contains(" charm")) {
					luckyPlayer.getSocialManager().sendGameMessage("<col=ff0000>Untradeable drop: " + item.getName() + ".");
				}
	...
	public void sendDrop(Player player, Drop drop) {
		...
		if (doubleDrop && (drop.getRate() == 100 || item.getDefinitions().isStackable()
