...
public class Spiritvulatrice extends Familiar {

	private static final long serialVersionUID = 6646329612349833935L;
	private int chocoTriceEgg;

	public Spiritvulatrice(Player owner, Pouch pouch, WorldTile tile, int mapAreaNameHash, boolean canBeAttackFromOutOfArea) {
	...
	public boolean submitSpecial(Object object) {
		...
		if (target instanceof Player) {
			...
			int level = playerTarget.getSkills().getLevelForXp(Skills.MAGIC);
			int drained = 3;
			if (level - drained > 0)
				drained = level;
			playerTarget.getSkills().drainLevel(Skills.MAGIC, drained);
		}
