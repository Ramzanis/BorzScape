...
/**
 * Contains the spoils for a {@link Player} when they defeat a mob.
 *
 * @author Andreas
 * @version 1.0
 * @since 10/07/2018
...
public final class MobRewardGenerator {
	 ...
	 */
	private static final String DROP_TABLE_PATH = "out/production/Server592/com/rs/game/npc/drops/droptable";
	...
	private boolean roll(Player player, int max) {
		...
		if (random <= 0) {
			if (row > 0) {
				player.getSocialManager().sendGameMessage("<col=ff7000>Your ring of wealth shines more brightly!</col>");
			}
			return true;
		}
		return false;
