...
public class DuelArena extends Controller {
	...
	public void endDuel(final Player victor, final Player loser, boolean isDraw) {
		...
		if (rules != null && !rules.hasRewardGiven()) {
			...
			Logger.globalLog(victor.getUsername(), victor.getIP(), new String(" completed the duel with " + loser.getUsername() + " previous items are: " + Arrays.toString(victor.getLastDuelRules().getStake().getShiftedItem()) + " new items are " + Arrays.toString(loser.getLastDuelRules().getStake().getShiftedItem())));
