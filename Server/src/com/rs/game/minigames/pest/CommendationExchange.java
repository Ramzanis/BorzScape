...
public class CommendationExchange {// 1875 TODO
	...
	private static final int[] SKILLS = { Skills.STRENGTH, Skills.DEFENCE, Skills.CONSTRUCTION, Skills.RANGE, Skills.MAGIC, Skills.PRAYER };
	...
	private static void refreshPoints(Player player) {
		player.getVarsManager().sendVarBit(2086, player.getPestPoints());
	}
	...
	private static void addVoidItem(Player player, int index) {
		...
		player.getDialogueManager().startDialogue("ItemMessage", "You exchange " + cost + " commendation points for a " + ItemDefinitions.getItemDefinitions(voidItem).name.toLowerCase() + ".", voidItem);
	}
