...
public final class BrimhavenAgility extends Controller {
	...
	private static class BladesManager extends TimerTask {
		...
		public void run() {
			/*
			 * for (Player target : players) { for (WorldObject object: World.getRegion(target.getRegionId()).getObjects()) { if
			 * (object.getId() == 3569 || object.getId() == 3568) { target.getPackets().sendObjectAnimation(object, new Animation(1)); }
			 * } }
			 */
		}
	}
