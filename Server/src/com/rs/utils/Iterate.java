	...
	public static final <T> void classes(String path, Consumer<T> iterate){
		...
		try {
			...
			for (File f : files) {
				...
				final String packagePath = f.getPath().replace("\\", ".").replace("out/", "").replace("production/.", "").replace("Server592/.", "");
				final Class<?> c = Class.forName(packagePath.substring(0, packagePath.length() - 6).replace('/', '.').replace("out.", "").replace("production.", "").replace("Server592.", ""));
