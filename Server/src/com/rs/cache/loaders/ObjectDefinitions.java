...
public class ObjectDefinitions {
	...
	public static ObjectDefinitions getObjectDefinitions(int id) {
		...
		if (def == null) {
			...
			def.method3287();
			/*
			 * if(def.name.equalsIgnoreCase("bank booth") || def.name.equalsIgnoreCase("counter")) { def.notCliped = false;
			 * def.projectileCliped = true; if (def.clipType == 0) def.clipType = 1; } else if (DungeonUtils.isDoor(id) ||
			 * DungeonUtils.isBossDoor(id)) { def.notCliped = false; def.projectileCliped = true; if (def.clipType == 0) def.clipType =
			 * 1; } if (def.notCliped) { def.projectileCliped = false; def.clipType = 0; }
			 */
